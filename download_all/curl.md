...
page=1
# curl -kv -H"Cookie:$COOKIE" 'https://www.gushiwen.cn/user/collect.aspx?type=s&p=1&id=5148311&sort=t' -O "$page.html"
lists=(`cat "$page.html" | grep -oP 'href="\K[^"]+' | sed 's/.aspx//')

echo "lists:$lists"
for((i=0;i<${#lists[@]};i++))
do
    # download detail
    echo "download ${lists[$i]}"
    curl -O "${lists[$i]}.html" https://www.gushiwen.cn/${lists[$i]}.aspx
    cat "${lists[$i]}.html" | grep -oP '(?<=">).*(?=https)' > "${lists[$i]}.txt"
    sleep 1
    exit 1
done
